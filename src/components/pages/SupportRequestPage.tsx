...
import { CheckCircle2, Clock3, FileText, Headphones, MessageCircle, Paperclip, Send, ShieldCheck, Upload, X } from 'lucide-react';
        ...
        {submitted ? (
          ...
          <form onSubmit={submit} className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.55fr)_minmax(20rem,.75fr)]">
            <Card className="rounded-[2rem] border-border/70 p-5 shadow-[0_18px_50px_rgba(10,38,68,.07)] sm:p-8">
              ...
              <div className="mt-5"><Label htmlFor="support-message">Message</Label><Textarea id="support-message" className="mt-2 min-h-52 resize-y" value={message} onChange={(event) => setMessage(event.target.value)} placeholder="What happened, when did it happen, and what outcome were you expecting?" /><p className="mt-2 text-xs text-muted-foreground">Include dates, amounts, and names when they help explain the issue.</p></div>
            </Card>

            <aside className="grid gap-4 lg:sticky lg:top-20">
              <Card className="rounded-[1.75rem] border-border/70 p-5 shadow-sm sm:p-6">
                ...
                <label htmlFor="support-attachments" className="mt-4 flex min-h-44 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-primary/35 bg-primary/[0.035] px-4 py-6 text-center transition hover:border-primary/60 hover:bg-primary/[0.06]" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); addAttachments(Array.from(event.dataTransfer.files)); }}><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><Upload size={20} /></span><span className="mt-3 text-sm font-semibold">Choose files or drag them here</span><span className="mt-1 text-xs leading-5 text-muted-foreground">PDF, JPG, PNG or WebP<br />5 MB maximum per file</span></label>
              ...
              <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.045] p-4"><div className="flex gap-3"><ShieldCheck size={18} className="mt-0.5 shrink-0 text-emerald-600" /><div><p className="text-sm font-semibold">Keep sensitive details private</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Never send your password, PIN, OTP, full card number, or recovery code.</p></div></div></div>
              <div className="flex gap-3 rounded-2xl border bg-muted/20 p-4"><Clock3 size={18} className="mt-0.5 shrink-0 text-primary" /><div><p className="text-sm font-semibold">What happens next?</p><p className="mt-1 text-xs leading-5 text-muted-foreground">We create a reference and continue the conversation in Messages.</p></div></div>
              <Button type="submit" className="h-12 w-full rounded-xl text-sm"><Send size={16} /> Submit support request</Button>
            </aside>
          </form>
        )}
      </div>
