...
export const authApi = {
  ...
  getLoginHistory: async () => {
    return await httpClient.get(endpoints.auth.loginHistory);
  },

  /**
   * Search for users (customers)
   */
  searchUsers: async (query: string, role?: string, limit?: number) => {
    ...
    return httpClient.get(
      endpoints.auth.searchUsers + (queryString ? `?${queryString}` : '')
    );
  },
  ...
  getNotificationPreferences: async () => {
    return await httpClient.get(endpoints.auth.notificationPreferences);
  },
  ...
  updateNotificationPreferences: async (data: {
    ...
    return await httpClient.put(endpoints.auth.notificationPreferences, data);
  },
  ...
  updateSecuritySettings: async (data: {
    ...
    return await httpClient.put(endpoints.auth.securitySettings, data);
  },
  ...
  generate2FASecret: async () => {
    return await httpClient.post(endpoints.auth.generate2FA);
  },
  ...
  verify2FASetup: async (token: string) => {
    return await httpClient.post(endpoints.auth.verify2FA, { token });
  },
   ...
   */
  disable2FA: async () => {
    return await httpClient.post(endpoints.auth.disable2FA);
  },
