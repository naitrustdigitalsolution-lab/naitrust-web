...
const controls: Array<{ key: keyof Omit<PlatformFeatures, 'chinaWholesaleFocus'>; title: string; description: string; icon: typeof Store; core?: boolean }> = [
  ...
  { key: 'logistics', title: 'China to Nigeria logistics', description: 'Keep shipping, consolidation, customs and delivery operations available.', icon: Truck, core: true },
...
export function FeatureControlSection() {
  ...
  return <div className="space-y-5">
    <Card className="overflow-hidden rounded-3xl border-primary/15">
      ...
      <div className="grid gap-3 p-5 sm:grid-cols-2 sm:p-6"><FocusItem icon={PackageSearch} text="China wholesale sourcing" /><FocusItem icon={UserCheck} text="Verified sourcing agents" /><FocusItem icon={Globe2} text="Bilingual Order Rooms" /><FocusItem icon={Truck} text="China to Nigeria delivery" /></div>
    </Card>
