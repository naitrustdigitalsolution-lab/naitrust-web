...
import { ArrowLeft, ArrowRight, BadgeCheck, Building2, CheckCircle2, ClipboardList, Factory, Globe2, Languages, LockKeyhole, LogOut, MapPin, PackageCheck, ShieldCheck, Truck, UserCheck } from 'lucide-react';
...
const copy = {
  en: {
    ...
    seoDescription: 'Join Naitrust as a verified sourcing agent, China supplier or logistics company and work with Nigerian buyers through managed Order Rooms.',
    homeLabel: 'Go to Naitrust home', programme: 'Verified China sourcing network',
    ...
    verificationRequired: 'Naitrust verification required', bilingual: 'English and Chinese workflows',
    imageAlt: 'A sourcing professional in China reviewing product requirements for a Nigerian buyer',
    imageCaption: 'China operations. Nigerian buyers. One accountable workflow.',
    roleEyebrow: 'Choose your partner role', roleTitle: 'Build trusted China-to-Nigeria orders together.',
    roleIntro: 'Every partner applies separately and remains unverified until Naitrust completes the relevant checks.',
    agentTitle: 'Sourcing agents', agentText: 'Find suppliers, compare quotes, coordinate samples, inspect products and keep buyers updated.', agentAction: 'Apply as a sourcing agent',
    supplierTitle: 'China suppliers', supplierText: 'Receive clear requirements, submit quotes and update samples, production and handover progress.', supplierAction: 'Register a supplier',
    logisticsTitle: 'Logistics companies', logisticsText: 'Support China pickup, warehousing, freight, customs coordination and delivery to Nigeria.', logisticsAction: 'Apply as a logistics partner',
    accessEyebrow: 'How access works', accessTitle: 'Approval before opportunity.', accessIntro: 'Creating an application does not immediately activate a partner account.',
    applyTitle: 'Apply', applyText: 'Tell us who you are, where you operate and what services you provide.',
    verifyTitle: 'Get verified', verifyText: 'Naitrust reviews identity, business and operating information for your role.',
    roomsTitle: 'Work in Order Rooms', roomsText: 'Approved partners receive access to buyer requests, evidence and order updates.',
    approvedTitle: 'Already approved by Naitrust?', approvedText: 'Use the email and access code issued through your verified contact channel.', portalAction: 'Open partner portal',
    navHome: 'Home', navBuyers: 'For Nigerian Buyers', navPartners: 'Partners', navAbout: 'About', navContact: 'Contact',
  },
  'zh-CN': {
    ...
    seoDescription: '作为已验证的采购代理、中国供应商或物流公司加入 Naitrust，通过受管理的订单室与尼日利亚买家合作。',
    homeLabel: '返回 Naitrust 首页', programme: '已验证的中国采购网络',
    hero: '加入经过验证的专业人士和企业网络，帮助尼日利亚个人和企业更安心地从中国批量采购。',
    verificationRequired: '必须通过 Naitrust 验证', bilingual: '中英文双语工作流程',
    imageAlt: '中国采购专业人士正在审核尼日利亚买家的产品需求',
    imageCaption: '中国本地执行，服务尼日利亚买家，全程记录清晰可查。',
    roleEyebrow: '选择合作伙伴类型', roleTitle: '共同建立可信赖的中尼采购订单。',
    roleIntro: '每类合作伙伴均须单独申请；在 Naitrust 完成相应审核前，账户将保持未验证状态。',
    agentTitle: '采购代理', agentText: '寻找供应商、比较报价、协调样品、验货，并及时向买家更新进展。', agentAction: '申请成为采购代理',
    supplierTitle: '中国供应商', supplierText: '接收清晰的采购需求，提交报价，并更新样品、生产及交接进度。', supplierAction: '注册成为供应商',
    logisticsTitle: '物流公司', logisticsText: '提供中国境内提货、仓储、货运、清关协调及尼日利亚配送服务。', logisticsAction: '申请成为物流合作伙伴',
    accessEyebrow: '访问权限流程', accessTitle: '先审核，后开展业务。', accessIntro: '提交申请不会立即开通合作伙伴账户。',
    applyTitle: '提交申请', applyText: '介绍您的身份、运营地点以及所提供的服务。',
    verifyTitle: '完成验证', verifyText: 'Naitrust 将根据合作伙伴类型审核身份、企业及运营信息。',
    roomsTitle: '在订单室开展工作', roomsText: '获批合作伙伴可查看买家需求、证据材料和订单进度。',
    approvedTitle: '已经通过 Naitrust 审核？', approvedText: '请使用发送至已验证联系方式的邮箱和访问码登录。', portalAction: '打开合作伙伴门户',
    navHome: '首页', navBuyers: '尼日利亚买家', navPartners: '合作伙伴', navAbout: '关于我们', navContact: '联系我们',
  },
};
...
export function PartnerNetworkPage() {
  ...
  return (
    <div className="min-h-svh bg-background">
      <SEOHead title={t.network} description={t.seoDescription} canonicalPath="/partners" />
      <header className="border-b bg-background/95 px-4 py-4 backdrop-blur sm:px-6 lg:px-8">
        <div className="relative mx-auto flex h-12 max-w-[90rem] items-center justify-between gap-3">
          <button type="button" onClick={() => navigate('/')} aria-label={t.homeLabel}><NaitrustLogo size="md" /></button>
          <nav className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-6 lg:flex" aria-label={locale === 'zh-CN' ? '主导航' : 'Primary navigation'}>
            {[{ label: t.navHome, path: '/' }, { label: t.navBuyers, path: '/customer' }, { label: t.navPartners, path: '/partners' }, { label: t.navAbout, path: '/about' }, { label: t.navContact, path: '/contact' }].map((item) => <button key={item.path} type="button" onClick={() => navigate(item.path)} className={`whitespace-nowrap text-sm transition hover:text-primary ${item.path === '/partners' ? 'font-semibold text-primary' : 'text-muted-foreground'}`}>{item.label}</button>)}
          </nav>
          <div className="flex items-center gap-2"><AppLanguageToggle compact /><Button variant="outline" className="hidden rounded-full sm:inline-flex" onClick={() => navigate('/partners/login')}><LockKeyhole size={15} /> {t.login}</Button></div>
        </div>
        <nav className="mx-auto mt-3 flex max-w-[90rem] gap-5 overflow-x-auto border-t pt-3 text-sm lg:hidden" aria-label={locale === 'zh-CN' ? '主导航' : 'Primary navigation'}>
          {[{ label: t.navHome, path: '/' }, { label: t.navBuyers, path: '/customer' }, { label: t.navPartners, path: '/partners' }, { label: t.navAbout, path: '/about' }, { label: t.navContact, path: '/contact' }].map((item) => <button key={item.path} type="button" onClick={() => navigate(item.path)} className={`shrink-0 whitespace-nowrap pb-1 ${item.path === '/partners' ? 'font-semibold text-primary' : 'text-muted-foreground'}`}>{item.label}</button>)}
        </nav>
      </header>
      <main>
        <section className="overflow-hidden bg-[#04162f] px-4 py-12 text-white sm:px-6 sm:py-16 lg:px-8 lg:py-20">
          <div className="mx-auto grid max-w-[90rem] items-center gap-10 lg:grid-cols-[.92fr_1.08fr] lg:gap-16">
            <div>
              <Badge className="w-fit border-white/15 bg-white/10 text-white"><Globe2 size={12} /> {t.programme}</Badge>
              <h1 className="mt-6 max-w-3xl text-4xl font-bold leading-[1.02] tracking-[-.05em] sm:text-5xl lg:text-6xl">{t.network}</h1>
              <p className="mt-5 max-w-xl text-base leading-7 text-white/70 sm:text-lg">{t.hero}</p>
              <div className="mt-8 flex flex-wrap gap-3"><Button className="h-11 rounded-full bg-white px-6 text-[#071b31] hover:bg-white/90" onClick={() => navigate('/partners/agent/apply')}><UserCheck size={16} /> {t.agent}</Button><Button variant="outline" className="h-11 rounded-full border-white/20 bg-white/[.07] px-6 text-white hover:bg-white/15 hover:text-white" onClick={() => navigate('/partners/login')}>{t.login} <ArrowRight size={15} /></Button></div>
              <div className="mt-7 flex flex-wrap gap-x-6 gap-y-2 text-xs text-white/60"><span className="flex items-center gap-2"><BadgeCheck size={15} className="text-emerald-400" /> {t.verificationRequired}</span><span className="flex items-center gap-2"><Languages size={15} className="text-sky-300" /> {t.bilingual}</span></div>
            </div>
            <div className="relative"><div className="absolute -inset-3 rounded-[2rem] border border-white/10 bg-white/[.04]" /><div className="relative overflow-hidden rounded-[1.6rem] border border-white/15 shadow-2xl shadow-black/30"><img src={pageImages.aboutHero.src} alt={t.imageAlt} className="aspect-[4/3] w-full object-cover" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#04162f] to-transparent p-6 pt-24"><p className="text-sm font-semibold">{t.imageCaption}</p></div></div></div>
          </div>
        </section>

        <section className="px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          <div className="mx-auto max-w-[90rem]"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[.18em] text-primary">{t.roleEyebrow}</p><h2 className="mt-3 text-3xl font-bold tracking-[-.04em] sm:text-4xl">{t.roleTitle}</h2><p className="mt-4 leading-7 text-muted-foreground">{t.roleIntro}</p></div>
            <div className="mt-10 grid gap-4 lg:grid-cols-3">
              <PartnerPath icon={UserCheck} title={t.agentTitle} text={t.agentText} action={t.agentAction} onClick={() => navigate('/partners/agent/apply')} featured />
              <PartnerPath icon={Factory} title={t.supplierTitle} text={t.supplierText} action={t.supplierAction} onClick={() => navigate('/partners/supplier/apply')} />
              <PartnerPath icon={Truck} title={t.logisticsTitle} text={t.logisticsText} action={t.logisticsAction} onClick={() => navigate('/partners/logistics/apply')} />
            </div>
          </div>
        </section>

        <section className="bg-[#f4f7fb] px-4 py-16 sm:px-6 sm:py-20 lg:px-8 dark:bg-card/40"><div className="mx-auto grid max-w-[90rem] gap-10 lg:grid-cols-[.75fr_1.25fr] lg:items-start"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-primary">{t.accessEyebrow}</p><h2 className="mt-3 text-3xl font-bold tracking-[-.04em] sm:text-4xl">{t.accessTitle}</h2><p className="mt-4 leading-7 text-muted-foreground">{t.accessIntro}</p></div><div className="grid gap-3 sm:grid-cols-3"><PartnerStep number="01" title={t.applyTitle} text={t.applyText} /><PartnerStep number="02" title={t.verifyTitle} text={t.verifyText} /><PartnerStep number="03" title={t.roomsTitle} text={t.roomsText} /></div></div></section>

        <section className="px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto flex max-w-[90rem] flex-col items-start justify-between gap-6 rounded-[2rem] bg-[#071a32] p-7 text-white sm:p-10 lg:flex-row lg:items-center"><div><h2 className="text-2xl font-bold sm:text-3xl">{t.approvedTitle}</h2><p className="mt-2 text-sm leading-6 text-white/60">{t.approvedText}</p></div><Button className="h-11 shrink-0 rounded-full px-6" onClick={() => navigate('/partners/login')}>{t.portalAction} <ArrowRight size={16} /></Button></div></section>
      </main>
    </div>
  );
}

function PartnerPath({ icon: Icon, title, text, action, onClick, featured = false }: { icon: typeof UserCheck; title: string; text: string; action: string; onClick: () => void; featured?: boolean }) {
  return <Card className={`group rounded-[1.5rem] p-6 transition hover:-translate-y-1 hover:shadow-xl ${featured ? 'border-primary/30 bg-primary/[.035]' : ''}`}><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Icon size={21} /></span><h3 className="mt-7 text-xl font-bold">{title}</h3><p className="mt-3 min-h-18 text-sm leading-6 text-muted-foreground">{text}</p><Button variant={featured ? 'default' : 'outline'} className="mt-6 w-full rounded-full" onClick={onClick}>{action} <ArrowRight size={15} /></Button></Card>;
}

function PartnerStep({ number, title, text }: { number: string; title: string; text: string }) {
  return <div className="rounded-2xl border bg-background p-5"><span className="text-xs font-bold text-primary">{number}</span><h3 className="mt-7 font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p></div>;
}

function PartnerTopbar({ locale, onLocale }: { locale: Locale; onLocale: (locale: Locale) => void }) {
  return <header className="mx-auto flex max-w-7xl items-center justify-between gap-4"><NaitrustLogo size="md" /><AppLanguageToggle compact /></header>;
}
