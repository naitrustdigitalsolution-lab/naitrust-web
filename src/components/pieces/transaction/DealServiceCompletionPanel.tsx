...
export function DealServiceCompletionPanel({ deal, viewerRole, hasDispute, onUploadEvidence }: { deal: SafeDealDetail; viewerRole: DealRole; hasDispute: boolean; onUploadEvidence: () => void }) {
  ...
  if (!['funded', 'released'].includes(deal.funding.status)) return null;
  ...
  return (
    <Card className="mb-4 overflow-hidden rounded-2xl border-primary/15 p-0 shadow-sm">
      <div className="flex items-start gap-3 p-4 sm:p-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            {completed ? (appConfig.isMock ? 'The work was approved locally. No provider payout was made.' : 'The buyer approved the work and the deal payment was released.')
              : awaitingReview ? (isProvider ? 'Your payment request is waiting for the buyer to review the evidence.' : 'The provider submitted work and requested payment. Review the evidence before deciding.')
                : deal.completion.status === 'changes_requested' ? (isProvider ? `The buyer requested changes: ${deal.completion.changesReason}` : 'You requested changes. The provider can update the work.')
                  : isProvider ? 'Add evidence of the completed work, then request payment when it is ready for review.' : (appConfig.isMock ? 'Review the work and evidence. Payment release is unavailable until the provider is connected.' : 'Payment remains protected until the provider submits work and you approve release.')}
          </p>
        </div>
      </div>
      ...
      {!completed && !hasDispute && (
        <div className="flex flex-col gap-2 border-t bg-muted/20 p-4 sm:flex-row sm:justify-end">
          ...
          {!isProvider && awaitingReview && <>
            ...
            {appConfig.isMock ? <p className="text-sm text-muted-foreground">Payment release is unavailable until the provider is connected.</p> : <Button className="rounded-full" onClick={() => setShowPin(true)}><CheckCircle2 size={15} /> Approve & release funds</Button>}
          </>}
        </div>
      )}
      ...
      <PinPromptModal open={showPin} onOpenChange={setShowPin} title="Release payment?" description="Confirm that you reviewed the submitted work and evidence and are satisfied." warning="Once released, this payment cannot be frozen or reversed from the Deal Room." onVerified={() => approveRelease.mutate(undefined, { onSuccess: () => toast.success('Payment released to the provider.'), onError: (error) => toast.error(error instanceof Error ? error.message : 'Could not release payment.') })} />
    </Card>
  );
}
